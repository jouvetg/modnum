...
BLEU = "#cfe6fb"      # case déjà remplie
BLEU_BORD = "#3b82c4"
ORANGE = "#f59e2b"    # case remplie à cette itération
GRIS = "#f1f1f1"      # case encore à zéro
GRIS_BORD = "#b5b5b5"
TEXTE = "#222222"
...
def ligne(ax, y, it, label):
    ...
    for k, i in enumerate(indices):
        ...
        ax.text(x + W / 2, y + H / 2, f"{val}", ha="center", va="center", fontsize=12,
                family=MONO, color=TEXTE if rempli else "#9a9a9a",
    ...
    if it >= 1:
        ...
        a = FancyArrowPatch((xcase(k - 1) + W / 2, y + H), (xcase(k) + W / 2, y + H),
                            ...
                            mutation_scale=14, lw=1.6, color="#b86a00")
        ax.add_patch(a)
        ax.text(xcase(k), y + H + 0.62, "+ M_epargne", ha="center", va="bottom",
                fontsize=11, family=MONO, color="#b86a00")
...
fig.tight_layout()
for ext in ("svg", "png"):
    fig.savefig(f"01_exercice/fig/remplissage_vecteur.{ext}", dpi=150, facecolor="white")
